...
CREATE UNIQUE INDEX unique_active_username   ON users(lower(user_name))   WHERE deleted_at  IS NULL;
CREATE UNIQUE INDEX unique_active_user_email   ON users(lower(user_email))   WHERE deleted_at IS NULL;
...
create unique index unique_resource_name on resources(lower(resources_name));


CREATE TABLE role_permissions(
    role_permission_id uuid primary key,
    user_role_id uuid not null references user_roles(user_role_id) on delete cascade,
    resources_id uuid not null references resources(resources_id) ,
...
CREATE INDEX role_permission on role_permissions(user_role_id,resources_id,action_id);
